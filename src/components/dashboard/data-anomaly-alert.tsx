...
import { AlertTriangle, RefreshCw, ShieldAlert, ArrowLeftRight } from 'lucide-react';
    ...
    try {
      ...
      if (orphanedGrns.length > 0) {
        foundAnomalies.push({
          ...
          description: `تم اكتشاف استلامات بضاعة تم حذف قيودها المحاسبية، مما يسبب عدم تطابق بين المخزن والحسابات.`,
          link: '/dashboard/warehouse/grns'
        });
      }

      // 2. فحص فجوات سندات القبض
      ...
      if (orphanedReceipts.length > 0) {
        foundAnomalies.push({
          ...
          description: `يوجد سندات قبض بدون قيود مالية، الإيرادات المسجلة دفترياً غير دقيقة.`,
          link: '/dashboard/accounting/cash-receipts'
        });
      }

      // 3. فحص فجوات سندات الصرف
      ...
      if (orphanedPayments.length > 0) {
        foundAnomalies.push({
          ...
          description: `يوجد سندات صرف تم حذف قيودها، المصاريف والتدفقات النقدية غير متوازنة.`,
      ...
      {anomalies.map((anomaly, idx) => (
        <Alert key={idx} variant="destructive" className="border-2 border-red-500 bg-red-50 dark:bg-red-950/20 shadow-lg shadow-red-100">
          ...
          <AlertTitle className="font-black text-red-700 dark:text-red-400 flex items-center justify-between">
            <span>تنبيه خلل بيانات: {anomaly.title}</span>
