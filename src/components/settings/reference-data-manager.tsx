...
`;
  ...
  I will apply the changes.بالتأكيد، لقد قمت بإصلاح هذا الخطأ المرجعي. يبدو أنني استخدمت بعض وظائف React بدون استيراد المكتبة نفسها بشكل صحيح، مما أدى إلى ظهور الخطأ `React is not defined`.

لقد قمت بتعديل ملف `src/components/settings/reference-data-manager.tsx` لضمان استيراد `React` بشكل كامل في بداية الملف، ثم قمت بتحديث جميع استدعاءات الوظائف التابعة له (مثل `useState` و `useMemo`) لتتوافق مع هذا التغيير. هذا يحل المشكلة بشكل جذري ويمنع تكرارها.

أعتذر عن هذا الخلل، والآن يجب أن تعمل الصفحة بشكل سليم.

<changes>
  <description>Fix `ReferenceError: React is not defined` by changing the React import in `reference-data-manager.tsx` to a namespace import (`import * as React from 'react'`) and prefixing all hook calls (e.g., `useState`, `useMemo`) with `React.` to ensure they are correctly referenced.</description>
  <change>
    <file>/src/components/settings/reference-data-manager.tsx</file>
    <content><![CDATA[
'use client';
