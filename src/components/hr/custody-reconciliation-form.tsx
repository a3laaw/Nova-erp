          ...
          >
            {p.file.type.startsWith('image/') ? (
              ...
              <div className="flex flex-col items-center justify-center h-full bg-indigo-50">
                ...
                <span className="text-[7px] font-black text-indigo-800 truncate px-1 w-full text-center">
                  {p.file.name}
                </span>
              </div>
            )}
...
export function CustodyReconciliationForm() {
  ...
  const { data: employees = [], loading: employeesLoading } = useSubscription<Employee>(
    firestore,
    'employees',
    [where('status', '==', 'active')]
  );
  ...
  const { data: accounts = [] } = useSubscription<Account>(firestore, 'chartOfAccounts', [
    orderBy('code'),
  ]);
  ...
  useEffect(() => {
    ...
    const fetchBalance = async () => {
      try {
        ...
        const custodyAcc = accounts.find(
          (a) => a.parentCode === '110102' && a.name.includes(emp?.fullName || '')
        );
        if (!custodyAcc) {
          setCustodyBalance(0);
          return;
        }
        const jesSnap = await getDocs(
          query(collection(firestore, 'journalEntries'), where('status', '==', 'posted'))
        );
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    if (totalSpent > custodyBalance && currentUser.role !== 'Admin') {
      toast({
        variant: 'destructive',
        title: 'تجاوز الرصيد',
        description: 'لا يمكنك تسوية مبلغ أكبر من المتوفر في عهدتك.',
      });
      return;
    }
  ...
  return (
    <Card
      className={cn(
        'max-w-5xl mx-auto rounded-[2.5rem] border-none shadow-2xl overflow-hidden',
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <CardTitle className="text-2xl font-black text-slate-900">
                تسوية عهدة نقدية (منصة الميدان)
              </CardTitle>
              <CardDescription className="text-base font-medium">
                وثق مصروفاتك وارفع صور الفواتير؛ سيقوم المحاسب بربطها محاسبياً لاحقاً.
              </CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent className="p-8 space-y-10">
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-center px-2">
              ...
              <Badge
                variant="outline"
                className="font-bold border-primary/20 text-primary bg-primary/5 px-4 h-7 rounded-full"
              >
            ...
            <div className="space-y-6">
              {fields.map((field, index) => {
                ...
                return (
                  <div
                    key={field.id}
                    className="grid grid-cols-1 lg:grid-cols-12 gap-6 p-8 bg-white border-2 rounded-[2.5rem] shadow-sm hover:shadow-md transition-all group relative"
                  >
                    <div className="lg:col-span-4 space-y-4">
                      <div className="grid gap-1.5">
                        <Label className="text-[10px] font-black text-muted-foreground uppercase pr-1">
                          بيان المصروف *
                        </Label>
                        <Input
                    ...
                    <div className="lg:col-span-2 flex flex-col justify-center gap-1.5 px-4 lg:border-r lg:border-l">
                      <Label className="text-[10px] font-black text-primary uppercase text-center">
                        المبلغ المستحق
                      </Label>
                      <Input
                        type="number"
                    ...
                    <div className="lg:col-span-5 space-y-2">
                      <Label className="text-[10px] font-black text-muted-foreground uppercase pr-1">
                        الفواتير والمرفقات (بالبكرة)
                      </Label>

                      <div className="flex gap-3 items-center">
                        <div className="w-20 h-24 rounded-2xl border-2 border-dashed flex flex-col items-center justify-center gap-1 shrink-0 transition-all cursor-pointer relative bg-muted/30 border-muted-foreground/20 hover:bg-muted/50">
                          <UploadCloud className="h-5 w-5 text-primary opacity-40" />
                          <span className="text-[8px] font-black text-primary text-center leading-tight">
                            إدراج
                          </span>
                          <input
                            type="file"
            ...
            <Button
              ...
              variant="outline"
              onClick={() =>
                append({ id: generateId(), description: '', amount: '', projectId: '', clientId: '' } as any)
              }
              disabled={isSaving}
          ...
          <div className="grid gap-3 p-8 bg-white/40 rounded-[2.5rem] border-2 border-dashed border-muted-foreground/10">
            ...
            <Textarea
              {...register('notes')}
              placeholder="أدخل أي توضيحات إضافية حول المصروفات المذكورة..."
              className="rounded-2xl border-none shadow-inner text-base p-6 min-h-[120px]"
              rows={3}
              disabled={isSaving}
            />
          </div>
        ...
        <CardFooter className="flex justify-between gap-4 p-10 border-t bg-muted/10 rounded-b-[2.5rem]">
          <div className="space-y-1">
            ...
            <p
              className={cn(
                'text-3xl font-black font-mono tracking-tight transition-colors',
