...
// ─── مولد ID آمن (نفس المستخدم في use-boq-save) ───
...
const BoqItemRowRenderer = React.memo(
  ({
    ...
    const quantity = useWatch({
      control,
      name: `items.${node._index}.quantity`,
    });
    const price = useWatch({
      control,
      name: `items.${node._index}.sellingUnitPrice`,
    });
    const isHeader = useWatch({
      control,
      name: `items.${node._index}.isHeader`,
    });
    const itemId = useWatch({
      control,
      name: `items.${node._index}.itemId`,
    });
    ...
    const handleMasterItemSelect = React.useCallback(
      (value: string) => {
        ...
        const selectedItem = allMasterItems.find(
          (i) => i.value === value
        );
        if (selectedItem) {
          setValue(`items.${node._index}.itemId`, value, {
            shouldDirty: true,
          });
          setValue(
            `items.${node._index}.description`,
            selectedItem.label,
            { shouldDirty: true }
          );
          const shouldBeHeader =
            selectedItem.isHeader || masterItemsMap.has(value);
          setValue(`items.${node._index}.isHeader`, shouldBeHeader, {
            shouldDirty: true,
          });
          setValue(
            `items.${node._index}.unit`,
            shouldBeHeader ? '' : selectedItem.unit || 'مقطوعية',
            { shouldDirty: true }
          );
        }
      },
    ...
    const findLastDescendantIndex = React.useCallback(
      (parentIndex: number): number => {
        ...
        for (let i = parentIndex + 1; i < watchedItems.length; i++) {
          if (
            watchedItems[i].parentId ===
            watchedItems[parentIndex].uid
          ) {
            lastIndex = findLastDescendantIndex(i);
          } else if (
            watchedItems[i].level <= watchedItems[parentIndex].level
          ) {
            break;
          }
        }
    ...
    const handleAddClick = React.useCallback(
      (headerMode: boolean) => {
        const lastDescendantIndex = findLastDescendantIndex(
          node._index
        );
      ...
      <React.Fragment>
        <TableRow
          className={cn(
            'transition-colors',
            isHeader
              ? 'bg-muted/40 font-bold border-b-2'
              : 'hover:bg-muted/20',
        ...
        >
          ...
          >
            <div className="flex flex-col gap-2 py-1">
              <InlineSearchList
                ...
                options={
                  masterItemsMap.get(parentReferenceId) || []
                }
                placeholder={
                  masterItemsLoading
                    ? 'تحميل...'
              ...
              <Textarea
                ...
                rows={1}
                className={cn(
                  'text-sm mt-1 min-h-[38px] border-muted focus:border-primary transition-all',
                  itemError?.description
                    ? 'border-destructive'
                    : ''
                )}
          ...
          <TableCell className="px-1">
            <Input
              ...
              step="0.001"
              {...register(
                `items.${node._index}.sellingUnitPrice`
              )}
          ...
          <TableCell className="text-left font-mono font-bold border-r bg-muted/10 px-3">
            <div
              className={cn(
                ...
                isHeader
                  ? 'text-primary border-b-2 border-primary/20'
                  : 'text-foreground'
              )}
            >
...
export function BoqForm({
  ...
  const masterItemsConstraints = React.useMemo(
    () => [orderBy('name')],
    []
  );
  const { data: masterItemsData, loading: masterItemsLoading } =
    useSubscription<BoqReferenceItem>(
      firestore,
      'boqReferenceItems',
      masterItemsConstraints
    );
  ...
  const { boqTree, grandTotal } = React.useMemo(() => {
    ...
    const itemsWithChildren: BoqItemWithChildren[] = items.map(
    ...
    const map = new Map(
      itemsWithChildren.map((item) => [item.uid, item])
    );
    ...
    items.forEach((item: any) => {
      if (!item.isHeader) {
        totalSum +=
          ...
          (Number(item.sellingUnitPrice) || 0);
      }
    });
  ...
  const handleAddItem = React.useCallback(
    (
      ...
      const parentItem = watchedItems?.find(
        (f: any) => f.uid === parentId
      );
          ...
          <div className="flex justify-between items-center max-w-full px-4 mx-auto">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <CardDescription className="flex items-center gap-2">
                  ...
                  إدارة الحصر والتسعير بنظام الهيكل الإنشائي
                  (WBS)
                </CardDescription>
              </div>
            </div>
            <div className="flex items-center gap-8 bg-muted/30 px-6 py-3 rounded-2xl border border-muted-foreground/10">
              ...
              <div className="flex flex-col items-start">
                ...
                <div className="text-2xl font-bold font-mono text-foreground/80">
                  {fields.length}
                </div>
              </div>
            </div>
        ...
        <CardContent className="p-0 max-w-full mx-auto mt-6 px-4 pb-32">
          <div className="grid md:grid-cols-3 gap-8 p-8 mb-6 border rounded-3xl bg-card shadow-sm">
            <div className="grid gap-2">
              ...
              <Input
                {...register('name')}
                placeholder="مثال: جدول كميات فيلا السيد محمد"
                className={cn(
                  'h-11 text-lg border-2',
              ...
              <Controller
                ...
                render={({ field }) => (
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="تقديري">
                        تقديري
                      </SelectItem>
                      <SelectItem value="تعاقدي">
                        تعاقدي
                      </SelectItem>
                      <SelectItem value="منفذ">
                        منفذ
                      </SelectItem>
                    </SelectContent>
                  </Select>
              ...
              >
                ...
                <TableHeader className="bg-muted/80 backdrop-blur-sm">
                  <TableRow className="hover:bg-transparent border-b-2 h-14">
                    <TableHead className="text-center font-bold text-xs uppercase px-1">
                      م
                    </TableHead>
                    <TableHead className="font-bold px-2">
                      بيان الأعمال التفصيلي
                    </TableHead>
                    ...
                    <TableHead className="text-left font-bold border-r px-3">
                      الإجمالي
                    </TableHead>
                    <TableHead className="font-bold px-2">
                      ملاحظات
                    </TableHead>
                    <TableHead className="text-center font-bold border-r px-1">
                      إجراءات
                    </TableHead>
                  </TableRow>
                </TableHeader>
                    ...
                    <TableRow>
                      ...
                      >
                        <div className="flex flex-col items-center justify-center gap-4 text-muted-foreground">
                          ...
                          <p>
                            الجدول فارغ حالياً. ابدأ بإضافة
                            قسم رئيسي من الأسفل.
                          </p>
                        </div>
                      </TableCell>
                ...
                <TableFooter className="bg-primary/5">
                  <TableRow className="font-black text-xl hover:bg-transparent">
                    <TableCell
                      colSpan={5}
                      className="text-right py-8 px-8"
                    >
                      الإجمالي العام لجدول الكميات:
                    </TableCell>
                    <TableCell className="text-left font-mono text-primary py-8 border-r bg-primary/5 px-3">
