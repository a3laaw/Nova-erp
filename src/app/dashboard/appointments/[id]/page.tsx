...
export default function AppointmentDetailsPage() {
    ...
    // جلب مراحل العمل بناءً على القسم الهندسي للمعاملة
    useEffect(() => {
        if (!firestore || !tenantId || !transaction?.departmentId) return;
    ...
    // محرك تحديث إنجاز الزيارة والربط المالي (The Golden Chain)
    const handleUpdateVisitStatus = async () => {
        ...
        try {
            ...
            const txSnap = await transaction_ref_read(); // دالة محاكاة للقراءة
            
            // تحديث حالة المرحلة في المعاملة
            if (transaction) {
                ...
                // الربط المالي: البحث عن دفعة مرتبطة
                let financeComment = "";
                ...
                // توثيق محضر الاجتماع في التايم لاين
    ...
    // وظيفة وهمية للقراءة لضمان توافق المنطق
    async function transaction_ref_read() {
        return { exists: () => true };
    }
                ...
                <CardContent className="p-10 space-y-10">
                    <div className="grid grid-cols-2 gap-8">
                        <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100 shadow-inner">
                            <Label className="text-[10px] uppercase font-black text-slate-400 block mb-2">تاريخ الموعد المجدول</Label>
                        ...
                        <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100 shadow-inner">
                            <Label className="text-[10px] uppercase font-black text-slate-400 block mb-2">وقت الزيارة</Label>
                    ...
                    {!appointment.workStageUpdated && appointment.transactionId ? (
                        <div className="p-8 border-4 border-dashed border-primary/20 bg-primary/5 rounded-[2.5rem] space-y-6 animate-in zoom-in-95 duration-500">
                            ...
                            <div className="grid gap-3">
                                <Label className="font-black text-slate-700 pr-2">ملخص الاجتماع / الملاحظات الفنية</Label>
                                <Textarea 
                                    value={notes} 
                            ...
                            >
                                ...
                                اعتماد الإنجاز وتحديث المالية
                            </Button>
                        </div>
                    ) : appointment.workStageUpdated ? (
                        <Alert className="bg-green-50 border-green-200 rounded-[2rem] py-8 shadow-xl shadow-green-100/50">
                            <Check className="h-8 w-8 text-green-600"/>
                            <AlertTitle className="text-green-800 font-black text-2xl mb-2">تم توثيق الزيارة بنجاح</AlertTitle>
                            <AlertDescription className="text-green-700 font-bold text-lg leading-relaxed">
                                تم إغلاق ملف الزيارة، تحديث سجل العميل التاريخي، وتفعيل الدفعات المالية المرتبطة بالمرحلة المنجزة آلياً.
                            </AlertDescription>
                        </Alert>
                    ) : (
                        <div className="space-y-6">
                            <Alert variant="destructive" className="rounded-[2rem] border-4 border-dashed border-red-200 py-8 bg-red-50/50 shadow-inner">
                                ...
                                <AlertDescription className="text-red-800 font-bold text-lg leading-relaxed">
                                    {!appointment.clientId 
                                        ...
                                        : "يرجى ربط هذه الزيارة بإحدى الخدمات المفتوحة لهذا العميل لتتمكن من تحديث مراحل سير العمل."}
                                </AlertDescription>
                            </Alert>
                            
                            <div className="flex flex-col gap-4">
                                {!appointment.clientId ? (
                                    <Button asChild className="h-16 rounded-[2rem] font-black text-2xl gap-4 shadow-2xl shadow-primary/30 bg-primary hover:bg-black transition-all">
