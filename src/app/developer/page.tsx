...
/**
 * غرفة التحكم الرئيسية (Pearl Master Dashboard V34.0):
 * - تم تفتيح الواجهة تماماً بناءً على طلب المستخدم.
 * - تحسين تباين الخطوط لضمان القراءة (Black text on Light background).
 * - استبدال التحميل بدوائر بأشكال Skeleton ناعمة.
 */
  ...
  const handleActivateRequest = async () => {
    ...
    try {
        ...
        const response = await fetch('/api/manage-tenant-user', {
            ...
            body: JSON.stringify({ action: 'create_tenant_user', companyId, email: requestToActivate.email, password: tempPassword, displayName: requestToActivate.contactName, username: requestToActivate.username, role: 'Admin' })
        });
        const result = await response.json();
        if (!result.success) throw new Error(result.error);
        const batch = writeBatch(firestore);
        const companyRef = doc(firestore, 'companies', companyId);
        batch.set(companyRef, { name: requestToActivate.companyName, adminEmail: requestToActivate.email.toLowerCase().trim(), adminUsername: requestToActivate.username, activityType: requestToActivate.activity, status: 'active', subscriptionType: 'trial', maxUsersLimit: 5, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
  ...
  return (
    <div className="space-y-10" dir="rtl">
        ...
        <Card className="rounded-[3rem] border-none shadow-2xl overflow-hidden bg-white/80 backdrop-blur-xl border border-indigo-100/50">
            <CardHeader className="p-12 pb-10 bg-indigo-600/5 border-b border-indigo-100/50">
                <div className="flex flex-col md:flex-row justify-between items-center gap-8">
                    <div className="flex items-center gap-6">
                        <div className="p-5 bg-indigo-600 rounded-[2.2rem] shadow-xl shadow-indigo-100 border-2 border-white">
                            <Terminal className="h-10 w-10 text-white" />
                        </div>
                        <div className="text-right">
                            <CardTitle className="text-4xl font-black text-[#1e1b4b] tracking-tighter">غرفة التحكم الرئيسية</CardTitle>
                            <CardDescription className="text-slate-500 font-bold text-lg mt-1">إدارة المنشآت وتفعيل الحسابات والتحصيل المالي.</CardDescription>
                        </div>
                    </div>
                    <Button onClick={() => { setCompanyToEdit(null); setIsEditFormOpen(true); }} className="h-14 px-10 rounded-[1.8rem] font-black text-lg gap-3 bg-indigo-600 hover:bg-indigo-700 shadow-2xl shadow-indigo-200 border-none transition-all active:scale-95">
            ...
            <div className="flex justify-center mb-10">
                <TabsList className="bg-white p-1.5 rounded-[2.5rem] border border-indigo-100 h-16 w-full max-w-4xl shadow-xl">
                    ...
                    <TabsTrigger value="config" className="rounded-full flex-1 font-black gap-3 h-full text-base data-[state=active]:bg-indigo-600 data-[state=active]:text-white transition-all">
                        <Settings2 className="h-5 w-5" /> إعدادات النظام
                    </TabsTrigger>
            ...
            <TabsContent value="entities" className="animate-in fade-in slide-in-from-bottom-8 duration-700">
                <Tabs defaultValue="companies" className="space-y-6">
                    <div className="flex justify-start px-4">
                        <TabsList className="bg-white p-1 rounded-2xl h-11 border border-indigo-50 shadow-sm">
                            <TabsTrigger value="companies" className="rounded-xl px-8 font-black text-xs gap-2">المنشآت النشطة <Badge variant="secondary" className="h-4 px-1.5 text-[8px] bg-indigo-50 text-indigo-600 border-none">{companies.length}</Badge></TabsTrigger>
                            <TabsTrigger value="requests" className="rounded-xl px-8 font-black text-xs gap-2">طلبات الانضمام <Badge className="h-4 px-1.5 text-[8px] bg-orange-500 border-none">{requests.filter(r => r.status === 'pending').length}</Badge></TabsTrigger>
                        </TabsList>
                    </div>
                        ...
                        <Card className="rounded-[3.5rem] border-none shadow-2xl overflow-hidden bg-white">
                            <CardHeader className="bg-slate-50 border-b p-8 px-12 flex flex-row justify-between items-center">
                                <div className="relative w-full max-w-md">
                                    ...
                                    <Input placeholder="بحث باسم المنشأة..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 h-12 rounded-2xl bg-white border-2 border-slate-100 font-bold text-black" />
                                </div>
                                ...
                                <Table>
                                    <TableHeader className="bg-indigo-50">
                                        <TableRow className="border-none h-14">
                                            <TableHead className="px-12 font-black text-indigo-900 text-right">المنظمة والبريد الإداري</TableHead>
                                            <TableHead className="font-black text-indigo-900 text-center">نوع الاشتراك</TableHead>
                                            <TableHead className="font-black text-indigo-900 text-center">الاستحقاق المالي</TableHead>
                                            <TableHead className="text-left px-12 font-black text-indigo-900">تحكم</TableHead>
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                        {companiesLoading ? (
                                            Array.from({ length: 5 }).map((_, i) => (
                                                <TableRow key={i}><TableCell colSpan={4} className="px-12 py-6"><Skeleton className="h-12 w-full rounded-2xl" /></TableCell></TableRow>
                                            ))
                                        ) : filteredCompanies.length === 0 ? (
                                            <TableRow><TableCell colSpan={4} className="h-64 text-center text-muted-foreground italic font-black text-xl">لا توجد منشآت نشطة حالياً.</TableCell></TableRow>
                                        ) : (
                                            filteredCompanies.map(company => (
                                                <TableRow key={company.id} className="h-24 border-slate-50 group transition-all border-b last:border-0 hover:bg-indigo-50/30">
                                                    <TableCell className="px-12">
                                                        <div className="flex items-center gap-4">
                                                            ...
                                                            <div className="flex flex-col">
                                                                <span className="font-black text-xl text-black">{company.name}</span>
                                                                <span className="font-mono text-xs text-indigo-600 font-black opacity-60">{company.adminEmail}</span>
                                                            </div>
                                                        </div>
                                                    </TableCell>
                    ...
                    <TabsContent value="requests" className="m-0">
                         <Card className="rounded-[3.5rem] border-none shadow-2xl overflow-hidden bg-white">
                            <Table>
                                <TableHeader className="bg-indigo-50 h-14">
                                    <TableRow className="border-none">
                                        <TableHead className="px-10 font-black text-indigo-900 text-right">المنشأة والمالك</TableHead>
                                        <TableHead className="font-black text-indigo-900 text-center">التواصل</TableHead>
                                        <TableHead className="font-black text-indigo-900 text-center">التاريخ</TableHead>
                                ...
                                <TableBody>
                                    ...
                                    requests.map(req => (
                                        <TableRow key={req.id} className="h-24 group transition-all border-b last:border-0 hover:bg-indigo-50/30">
                                            <TableCell className="px-10">
                                                <div className="flex flex-col">
                                                    <span className="font-black text-xl text-black">{req.companyName}</span>
                                                    <span className="text-xs font-bold text-indigo-600 flex items-center gap-1"><User className="h-3 w-3"/> {req.contactName}</span>
                                                </div>
                                            </TableCell>
                                            <TableCell className="text-center">
                                            ...
                                            <TableCell className="text-left px-12">
                                                {req.status === 'pending' ? (
                                                    ...
                                                    >
                                                        <UserPlus className="h-4 w-4" /> تفعيل
                                                    </Button>
                                                ) : (
        ...
        <AlertDialog open={!!requestToActivate} onOpenChange={() => setRequestToActivate(null)}>
            <AlertDialogContent dir="rtl" className="rounded-[2.5rem] p-10 border-none shadow-2xl bg-white max-w-xl">
                <AlertDialogHeader>
                    ...
                    <AlertDialogTitle className="text-2xl font-black text-[#1e1b4b]">تفعيل المنشأة فوراً؟</AlertDialogTitle>
                    <AlertDialogDescription className="text-lg font-medium leading-relaxed mt-2 text-slate-600">
                        سيقوم النظام بتأسيس منشأة <strong>"{requestToActivate?.companyName}"</strong> وحقن حساب المدير آلياً.
                    </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter className="mt-10 gap-3">
                    ...
                    <AlertDialogAction onClick={handleActivateRequest} disabled={isProcessing === 'activating'} className="bg-green-600 hover:bg-green-700 rounded-xl font-black h-12 px-12 shadow-xl shadow-green-100 flex-1">
                        {isProcessing === 'activating' ? <Loader2 className="animate-spin h-5 w-5"/> : 'بدء التفعيل'}
                    </AlertDialogAction>
                </AlertDialogFooter>
            </AlertDialogContent>
        </AlertDialog>
        ...
        <AlertDialog open={!!companyToDelete} onOpenChange={() => setCompanyToDelete(null)}>
            <AlertDialogContent dir="rtl" className="rounded-[2.5rem] p-10 border-none shadow-2xl bg-white">
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription className="text-lg font-medium leading-relaxed mt-2 text-slate-600">
                        أنت على وشك حذف منشأة <strong>"{companyToDelete?.name}"</strong> بالكامل.
                    </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter className="mt-8 gap-3">
                    ...
                    <AlertDialogAction onClick={handleDeleteCompany} disabled={isProcessing === 'deleting'} className="bg-red-600 hover:bg-red-700 rounded-xl font-black h-12 px-12 shadow-xl shadow-red-200">
                        {isProcessing === 'deleting' ? <Loader2 className="animate-spin h-4 w-4"/> : 'تأكيد الحذف'}
                    </AlertDialogAction>
                </AlertDialogFooter>
            </AlertDialogContent>
        </AlertDialog>
