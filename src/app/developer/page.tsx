...
import { doc, updateDoc, collection, orderBy, query, getDocs, where } from 'firebase/firestore';
                        ...
                        <Table>
                            <TableHeader className="bg-[#1e1b4b] h-16">
                                <TableRow className="border-none hover:bg-transparent">
                                    ...
                                    <TableHead className="font-black text-indigo-100 text-base text-center">الترخيص والاشتراك</TableHead>
                                    <TableHead className="font-black text-indigo-100 text-base text-center">حصص المستخدمين</TableHead>
                            ...
                            <TableBody>
                                {loading ? (
                                    <TableRow>
                                        <TableCell colSpan={5} className="text-center p-40">
                                            <div className="flex flex-col items-center gap-4">
                                                <Loader2 className="animate-spin h-16 w-16 text-indigo-500" />
                                                <p className="font-black text-indigo-600 animate-pulse">جاري جلب قائمة التراخيص من السحابة...</p>
                                            </div>
                                        </TableCell>
                                    </TableRow>
                                ) : filteredCompanies.length === 0 ? (
                                    <TableRow><TableCell colSpan={5} className="h-80 text-center text-slate-300 italic font-black text-2xl tracking-widest uppercase">No Active Entities Found</TableCell></TableRow>
                                ) : (
                                    filteredCompanies.map(company => {
                                        ...
                                        return (
                                            <TableRow key={company.id} className={cn("h-32 hover:bg-indigo-50/50 border-slate-100 group transition-all", isExpired && "bg-red-50/30")}>
                                                ...
                                                <TableCell className="text-center">
                                                    <div className="flex flex-col items-center gap-2">
                                                        {company.subscriptionType === 'trial' ? (
                                                            <>
                                                                <Badge className={cn("px-4 py-1 rounded-full font-black text-[9px] tracking-widest", isExpired ? "bg-red-600 text-white" : "bg-orange-100 text-orange-700 border-orange-200")}>
                                                                    {isExpired ? 'DEMO EXPIRED' : 'DEMO MODE'}
                                                                </Badge>
                                                                {trialDate && (
                                                                    <div className="flex items-center gap-1 text-[10px] font-bold text-muted-foreground">
                                                                        <Timer className="h-3 w-3" />
                                                                        ينتهي: {format(trialDate, 'dd/MM/yyyy')}
                                                                    </div>
                                                                )}
                                                            </>
                                                        ) : (
                                                            <Badge className="bg-indigo-600 text-white px-4 py-1 rounded-full font-black text-[9px] tracking-widest border-2 border-white/20">PREMIUM SUBSCRIPTION</Badge>
                                                        )}
                                                    </div>
                                                </TableCell>
                                                <TableCell className="text-center">
                                                ...
                                                <TableCell className="text-center">
                                                    <Badge className={cn("px-6 py-1.5 rounded-full font-black text-[10px] tracking-widest border-2", company.isActive && !isExpired ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200')}>
