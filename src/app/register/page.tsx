        ...
        <CardContent className="p-10">
            <form onSubmit={handleSubmit} className="space-y-10">
                        ...
                        <div className="grid gap-5">
                            <div className="grid gap-2">
                                ...
                                <div className="relative group">
                                    ...
                                    <Input value={formData.companyName} onChange={e => setFormData(p => ({...p, companyName: e.target.value}))} className="pr-10 h-12 rounded-xl bg-white border-2 border-slate-200 shadow-inner font-black text-[#1e1b4b]" required placeholder="أدخل الاسم التجاري..." />
                                </div>
                            </div>
                            <div className="grid gap-2">
                                <Label className="text-[#1e1b4b] font-black text-xs pr-1">نوع النشاط الرئيسي</Label>
                        ...
                        <div className="grid gap-5">
                            <div className="grid gap-2">
                                ...
                                <div className="relative">
                                    ...
                                    <Input value={formData.contactName} onChange={e => setFormData(p => ({...p, contactName: e.target.value}))} className="pr-10 h-12 rounded-xl bg-white border-2 border-slate-200 font-black text-[#1e1b4b]" required placeholder="الاسم الثلاثي..." />
                                </div>
                            </div>
                            <div className="grid gap-2">
                ...
                <div className="grid gap-3 max-w-sm">
                    <Label className="text-[#1e1b4b] font-black text-xs pr-1">رقم التواصل (WhatsApp) *</Label>
                    <div className="relative">
                        <Phone className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-indigo-600" />
                        <Input value={formData.phone} onChange={e => setFormData(p => ({...p, phone: e.target.value}))} className="pr-10 h-12 rounded-xl bg-white border-2 border-slate-200 font-black text-[#1e1b4b] dir-ltr" required placeholder="+965 XXXX XXXX" />
                    </div>
                </div>

                <Button type="submit" disabled={isSaving} className="w-full h-16 rounded-[2.2rem] font-black text-xl gap-4 shadow-2xl bg-[#1e1b4b] text-white hover:bg-black transition-all border-b-8 border-black/40 active:translate-y-1 active:border-b-0">
