...
*   **الحل:** كيان وسيط اسمه `SupplierItem`.
    ...
    *   **القراءة الآلية (OCR):** سيتم استخدام تقنية OCR لقراءة ملفات PDF وتحويلها إلى بيانات شبه مهيكلة، ثم يقوم الذكاء الاصطناعي بمحاولة مطابقة أسماء الأصناف في الـ PDF مع قاعدة بيانات `SupplierItem` أو `Item` لتسريع عملية الإدخال.
...
*   **الخيار الأول (الأكثر قوة ومرونة):** استخدام مكتبات Python مع نماذج تعلم الآلة.
    ...
    *   **المنطق:** يتم بناء Flow في Genkit يستدعي دالة Python عبر API. هذه الدالة تقوم بقراءة الـ PDF، استخلاص جداول البيانات، ثم استخدام نموذج لغة كبير (مثل Gemini) لفهم سياق الجدول (أين عمود الصنف، الكمية، السعر) وتحويله إلى JSON منظم.
